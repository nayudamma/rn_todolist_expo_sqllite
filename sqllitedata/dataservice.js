...
export default function dataservie() {
  ...
  return {
    initializeDatabase() {
      ...
      db.transaction(tx => {
        tx.executeSql('drop table Sylabus_Tasklist1', null, (txObj, resultSet) => { console.log("---table droooped----"); }, (txObj, error) => { console.log("erroor in droooping table" + error) });
        console.log("table drooped");

        tx.executeSql('CREATE TABLE IF NOT EXISTS SylabusTasklist (id INTEGER PRIMARY KEY AUTOINCREMENT,' +
